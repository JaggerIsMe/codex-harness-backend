...
-- 补齐并启用 SYS_ADMIN、USER；补齐 16 项权限及 26 条内置角色权限关联。
...
-- 管理员：全部 16 项已定义的系统权限，不自动授予未知自定义权限。
...
-- 普通用户：10 项使用权限；不新增用户/设备/Skills/专家/MCP/模型管理权限。
INSERT INTO sys_role_permission(role_id, permission_id)
SELECT r.id, p.id FROM sys_role r CROSS JOIN sys_permission p
...
-- 验收：干净数据库应为 SYS_ADMIN=16、USER=10；原有自定义授权也会显示。
