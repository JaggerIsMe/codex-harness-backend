...
public class ModelConfigurationService {
    ...
    private ModelRuntimeDTO runtime(ModelConfigurationVersionPO v){ModelRuntimeSpecDTO s=read(v.getRuntimeSpec());ModelRuntimeDTO r=new ModelRuntimeDTO();copy(s,r);r.setSchemaVersion(3);r.setRuntimeMode(MANAGED_PROVIDER);r.setConfigurationId(v.getModelConfigurationId());r.setConfigurationVersionId(v.getId());r.setVersionNo(v.getVersionNo());r.setConfigurationCode(v.getConfigurationCode());r.setName(v.getName());r.setConfigDigest(v.getConfigDigest());r.setRuntimeKey(SecureDigests.sha256(v.getId()+":"+v.getConfigDigest()));r.setApiKey(cipher.decrypt(v.getEncryptedApiKey()));return r;}
