...
public interface McpConfigurationMapper {
    ...
    @Select("SELECT v.*,v.status version_status,c.status configuration_status FROM mcp_configuration_version v JOIN mcp_configuration c ON c.id=v.mcp_configuration_id WHERE c.status='ENABLED' AND v.status='ACTIVE' ORDER BY c.name,v.version_no DESC") List<McpConfigurationVersionPO> selectableVersions();
